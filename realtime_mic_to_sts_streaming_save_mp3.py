#!/usr/bin/env python3
"""
Real-time Microphone to STS Streaming Pipeline (Save MP3)
========================================================

- Listens to microphone, splits speech into natural phrases (VAD + silence detection)
- For each phrase, sends audio to STS API, collects the result
- Does NOT play back audio, but saves all output chunks into a single final MP3 file
...
PLAYBACK_DELAY = 0.2 # 200ms before playback (kept for timing, but no playback)
...
class RealtimeMicToSTS_SaveMP3:
    def __init__(self):
        self.audio = pyaudio.PyAudio()
        self.stream = None
        self.vad = webrtcvad.Vad(VAD_MODE)
        self.is_running = False
        self.output_dir = "123123"
        os.makedirs(self.output_dir, exist_ok=True)
        print(f"🎤 Real-time Mic→STS (Save MP3) initialized:")
            ...
            try:
                ...
                with open(temp_wav_path, "rb") as audio_file:
                    ...
                    if response.status_code == 200:
                        ...
                        # Save each chunk as a separate MP3 file in 123123
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        chunk_filename = os.path.join(self.output_dir, f"chunk_{phrase_num:03d}_{timestamp}.mp3")
                        with open(chunk_filename, 'wb') as f:
                            f.write(audio_data)
                        print(f"💾 Saved chunk {phrase_num} to {chunk_filename}")
                    else:
...
if __name__ == "__main__":
    pipeline = RealtimeMicToSTS_SaveMP3()
