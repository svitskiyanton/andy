#!/usr/bin/env python3
"""
Real-Time Voice Changer using ElevenLabs Speech-to-Speech API
...
class RealTimeVoiceChanger:
    def __init__(self, api_key: str, voice_id: str):
        ...
        self.input_queue = queue.Queue(maxsize=10)  # Reduced queue size
        self.output_queue = queue.Queue(maxsize=20)  # Reduced queue size
        ...
        self.channels = 1
        self.rate = 16000
        self.chunk_size = 2048  # Reduced for lower latency (128ms chunks)
        self.buffer_duration = 0.8  # Reduced to 0.8 seconds for faster processing
        ...
        self.min_interval = 0.5  # Minimum time between API calls
    ...
    def setup_audio(self):
        ...
        try:
            ...
            # Setup input stream with smaller buffer
            ...
            # Setup output stream with smaller buffer
            ...
            while self.running:
                try:
                    ...
                    if np.abs(audio_array).mean() > 100:  # Threshold for non-silence
                        ...
                        # For silence, add a small delay to prevent CPU overuse
                        time.sleep(0.01)
        ...
        while self.running:
            try:
                ...
                if not self.input_queue.empty():
                    ...
                    # Get audio buffer
                    audio_buffer = self.input_queue.get(timeout=0.1)
                    ...
                    # Send to ElevenLabs API with shorter timeout
...
def main():
    ...
    voice_changer = RealTimeVoiceChanger(api_key, voice_id)
    voice_changer.start()
