#!/usr/bin/env python3
"""
STS Streaming Pipeline DEBUG - Debug version with delays
Input Chunk → STS → Output Chunk → Playback
With detailed logging and delays to identify synchronization issues
"""
...
def cleanup_handler():
    ...
    if running:
        ...
        if audio_chunks:
            save_audio_chunks(audio_chunks, "sts_streaming_debug_output.mp3")
...
class STSStreamingPipelineDebug:
    def __init__(self, input_file=None):
        ...
        # DEBUG: Delays for synchronization
        self.VAD_DELAY = 0.5  # 500ms delay after VAD chunking
        self.STS_DELAY = 0.5  # 500ms delay after STS processing
        self.PLAYBACK_DELAY = 0.2  # 200ms delay before playback
        
        print(f"🎵 STS Streaming Pipeline DEBUG Configuration:")
    ...
    def _get_voice_id(self):
        ...
        # Default to Adam if not found - SAME AS test_pro_sts_streaming_enhanced_vad_fixed.py
    ...
    def detect_vad_boundaries(self, audio_segment):
        ...
        print("🔍 DEBUG: Starting VAD boundary detection...")
        ...
        print(f"🔍 DEBUG: Total frames: {total_frames}, Frame size: {frame_size}")
        ...
        print(f"🔍 DEBUG: Speech flags: {len(speech_flags)} frames")
        ...
        print(f"🔍 DEBUG: Found {len(boundaries)} VAD boundaries")
    ...
    def process_chunk_with_sts(self, chunk_audio, chunk_index, max_retries=3):
        ...
        print(f"🎵 DEBUG: Starting STS processing for chunk {chunk_index}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"🎵 DEBUG: Chunk {chunk_index} converted to WAV: {temp_file_path}")
                ...
                if response.status_code == 200:
                    ...
                    if audio_output:
                        ...
                        # DEBUG: Add delay after STS processing
                        print(f"⏳ DEBUG: Waiting {self.STS_DELAY}s after STS processing...")
    ...
    async def _smooth_audio_streaming(self):
        ...
        try:
            ...
            print("🎵 STS Streaming Pipeline DEBUG: Audio streaming initialized")
        ...
        try:
            while self.running:
                try:
                    ...
                    # DEBUG: Add delay before playback
                    print(f"⏳ DEBUG: Waiting {self.PLAYBACK_DELAY}s before playback...")
    ...
    def _play_audio_chunk_smooth(self, mp3_data, stream):
        ...
        try:
            print(f"🔊 DEBUG: Playing audio chunk ({len(mp3_data)} bytes)")
            ...
            print(f"✅ DEBUG: Audio chunk played successfully")
    ...
    def stream_audio_pipeline(self):
        ...
        print("🎤 STS Streaming Pipeline DEBUG: Starting VAD chunking...")
        ...
        # DEBUG: Add delay after VAD chunking
        print(f"⏳ DEBUG: Waiting {self.VAD_DELAY}s after VAD chunking...")
        ...
        for i, (start_ms, end_ms) in enumerate(chunks):
            ...
            print(f"\n🎵 STS Pipeline DEBUG: Processing chunk {chunk_count}/{len(chunks)} ({chunk_duration:.1f}s)")
            print(f"   Time range: {start_ms/1000:.1f}s - {end_ms/1000:.1f}s")
            
            # Extract chunk
            chunk_audio = audio[start_ms:end_ms]
            print(f"🔍 DEBUG: Extracted chunk {chunk_count} ({len(chunk_audio)}ms)")
            
            # Process with STS API
            audio_output = self.process_chunk_with_sts(chunk_audio, chunk_count)
            
            if audio_output:
                print(f"🔍 DEBUG: STS processing completed for chunk {chunk_count}")
                
                # Add to continuous buffer for playback
                try:
                    self.continuous_buffer.put_nowait(audio_output)
                    print(f"🔍 DEBUG: Added chunk {chunk_count} to playback buffer")
                except queue.Full:
                    print(f"⚠️ DEBUG: Playback buffer full, skipping chunk {chunk_count}")
                ...
                processed_chunks += 1
                print(f"✅ DEBUG: Chunk {chunk_count} processed successfully")
            else:
                print(f"❌ DEBUG: Failed to process chunk {chunk_count}")
            
            # Small delay between chunks for smooth streaming
            print(f"⏳ DEBUG: Waiting 0.1s between chunks...")
            time.sleep(0.1)
        
        print(f"🎵 STS Pipeline DEBUG: Processed {processed_chunks}/{chunk_count} chunks")
    
    async def start(self):
        """Start the STS streaming pipeline"""
        try:
            print("🎤🎵 STS Streaming Pipeline DEBUG: MP3 File → VAD Chunks → STS API → Playback")
            print("=" * 80)
            ...
            print(f"🎵 Model: {self.model_id}")
            print(f"🎵 Voice: {self.voice_id}")
            print("🔍 DEBUG: Added delays for synchronization analysis")
            print("=" * 80)
            ...
            print("🎤 STS Pipeline DEBUG: Starting VAD chunking and STS processing...")
            print("✅ STS Streaming Pipeline DEBUG started!")
            ...
            print("🔍 DEBUG: Watch for timing and synchronization issues")
            ...
            print("✅ STS Streaming Pipeline DEBUG completed")
            
        except Exception as e:
            print(f"❌ STS Streaming Pipeline DEBUG error: {e}")
        finally:
    ...
    def save_final_output(self):
        """Save final output file as combined MP3"""
        try:
            print("💾 Saving final output as combined MP3...")
            
            if not self.all_audio_chunks:
                print("⚠️ No audio chunks to save")
                return
            
            # Combine all MP3 chunks into one audio segment
            combined_audio = None
            
            for i, mp3_data in enumerate(self.all_audio_chunks):
                try:
                    # Convert MP3 bytes to AudioSegment
                    audio_chunk = AudioSegment.from_mp3(io.BytesIO(mp3_data))
                    
                    if combined_audio is None:
                        combined_audio = audio_chunk
                    else:
                        # Add silence between chunks for better separation
                        silence = AudioSegment.silent(duration=300)  # 300ms silence
                        combined_audio = combined_audio + silence + audio_chunk
                    
                    print(f"🔍 Added chunk {i+1} to combined audio ({len(audio_chunk)}ms)")
            ...
            if combined_audio:
                ...
                print(f"✅ Combined MP3 saved to: {self.OUTPUT_FILE}")
                print(f"📊 Total duration: {len(combined_audio)/1000:.1f}s")
                print(f"📊 Total chunks combined: {len(self.all_audio_chunks)}")
            else:
            ...
            try:
                ...
                print("✅ Fallback: Saved as raw audio chunks")
...
async def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="STS Streaming Pipeline DEBUG")
        parser.add_argument("input_file", help="Input MP3 file")
        parser.add_argument("--output", help="Output MP3 file", default="sts_streaming_debug_output.mp3")
        ...
        sts_pipeline = STSStreamingPipelineDebug(args.input_file)
