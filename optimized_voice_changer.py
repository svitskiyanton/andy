#!/usr/bin/env python3
"""
Optimized Real-time Voice Changer
Enhanced version with connection pooling and parallel processing
"""
...
class OptimizedVoiceChanger:
    def __init__(self):
        ...
        self.STT_CHUNK_DURATION = 3.0  # Reduced for faster response
        self.STT_CHUNK_SIZE = int(self.SAMPLE_RATE * self.STT_CHUNK_DURATION)
        self.SILENCE_THRESHOLD = 0.01
        self.SILENCE_DURATION = 1.0  # Reduced silence detection
        self.MIN_AUDIO_LENGTH = 0.8  # Reduced minimum audio length
        ...
        self.silence_buffer = deque(maxlen=int(self.SAMPLE_RATE * 2))
        self.last_processing_time = 0
        self.processing_cooldown = 0.3  # Reduced cooldown
        ...
        self.connection_pool_duration = 30  # Keep connection alive for 30s
    ...
    def start_streaming(self):
        """Start the optimized real-time voice changer"""
        ...
        print("🚀 Starting Optimized Real-time Voice Changer...")
        print("   Enhanced with connection pooling and parallel processing")
        ...
        try:
            ...
            print("\n🛑 Stopping optimized voice changer...")
            self.is_running = False
        ...
        print("✅ Optimized voice changer stopped")
    ...
    def _print_latency_report(self):
        ...
        print("\n" + "="*60)
        print("📊 OPTIMIZED LATENCY PERFORMANCE REPORT")
        print("="*60)
        ...
        if self.latency_stats['total_pipeline']:
            ...
            print(f"\n🚀 PERFORMANCE METRICS:")
            print(f"Average Response Time: {total_avg:.3f}s")
            print(f"Throughput: {self.total_phrases_processed / (session_duration / 60):.1f} phrases/min")
            print(f"Latency Consistency: {std_dev/total_avg*100:.1f}% CV")
        
        print("="*60)
    
    def _audio_recording_thread(self):
        """Optimized audio recording with faster silence detection"""
        try:
            ...
            print("🎤 Optimized recording started...")
            ...
            while self.is_running:
                try:
                    ...
                    if (current_time - self.last_processing_time > self.processing_cooldown and 
                        ...
                        # Shorter fallback duration
                        elif len(self.audio_buffer) >= int(self.SAMPLE_RATE * 6.0):
                            stt_chunk = list(self.audio_buffer)
                            self.audio_buffer.clear()
                            self.silence_buffer.clear()
                            ...
                            print(f"⏰ Max duration, processing {len(stt_chunk)/self.SAMPLE_RATE:.1f}s audio")
    ...
    def _stt_processing_thread(self):
        """Optimized STT processing with parallel execution"""
        print("🔍 Optimized STT processing started...")
        ...
        while self.is_running:
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(self._transcribe_audio, audio_data)
                    text = future.result(timeout=5.0)  # 5 second timeout
    ...
    def _tts_processing_thread(self):
        """Optimized TTS processing with connection pooling"""
        print("🎵 Optimized TTS processing started...")
        ...
        while self.is_running:
            try:
                ...
                asyncio.run(self._text_to_speech_stream_optimized(text, pipeline_start_time, tts_start_time))
    ...
    async def _text_to_speech_stream_optimized(self, text, pipeline_start_time, tts_start_time):
        ...
        try:
            ...
            if (self.tts_websocket is None or 
                ...
                self.tts_websocket = await websockets.connect(uri, extra_headers=headers)
    ...
    def _audio_playback_thread(self):
        ...
        print("🔊 Optimized audio playback started...")
...
def main():
    """Main function"""
    voice_changer = OptimizedVoiceChanger()
